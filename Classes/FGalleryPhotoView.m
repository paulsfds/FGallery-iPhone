...
- (id)initWithFrame:(CGRect)frame target:(id)target action:(SEL)action
{
	...
	[_button addTarget:self action:@selector(handleThumbTap:)
      forControlEvents:UIControlEventTouchUpInside];
	[self addSubview:_button];
	
    // save target and action for thumb tap
    _thumbTapTarget = target;
    _thumbTapAction = action;
    
	// create outline
	[self.layer setBorderWidth:4.0];
	[self.layer setBorderColor:[[UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:.5] CGColor]];
...
- (void)handleThumbTap:(id)sender
{
    if([self.photoDelegate photoGalleryIsInEditMode]) {
        [self setSelected:!self.selected];
    }
    else {
//#pragma clang diagnostic push
//#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [_thumbTapTarget performSelector:_thumbTapAction withObject:sender];
...
- (void)setFrame:(CGRect)theFrame
{
	...
	[_activity setCenter:CGPointMake(theFrame.size.width * .5f, theFrame.size.height * .5f)];
...
- (UIImage*)createHighlightImageWithFrame:(CGRect)rect
{
	...
	[blankLayer setBackgroundColor:[[UIColor colorWithRed:1.f green:1.f blue:1.f alpha:.25f] CGColor]];
...
-(void)setSelected:(BOOL)selected {
    ...
    if(self->_selected) {
        CGFloat margin = self.layer.borderWidth + 2.f;
        UIImageView * tickImView = [[UIImageView alloc] initWithFrame:CGRectMake(self.imageView.frame.size.width - margin - 20.f,
                                                                                 self.imageView.frame.size.height - margin - 20.f,
                                                                                 20.f, 20.f)];
        [tickImView setImage:[UIImage imageNamed:@"tick.png"]];
        [self.imageView addSubview:tickImView];
    }
    else {
        for(UIView *sub in [self.imageView subviews])
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (touch.tapCount == 2) {
		...
		if( _isZoomed ) 
		{
			...
			CGRect zoomRect = CGRectMake(touchCenter.x - zoomRectSize.width * .5f,
                                         touchCenter.y - zoomRectSize.height * .5f,
                                         zoomRectSize.width, zoomRectSize.height);
